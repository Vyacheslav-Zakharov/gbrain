...
describe('embedMultimodal — openai-compat routing (#875)', () => {
  ...
  test('D12 — Avers R1 default embedding_dimensions 768 applies when not explicitly set', async () => {
    // configureGateway normalizes embedding_dimensions to DEFAULT_EMBEDDING_DIMENSIONS
    // when unset. Avers R1 uses Google 768d. LiteLLM recipe's default_dims=0 so we fall back
    ...
    configureGateway({
      ...
      // intentionally NO embedding_dimensions → falls back to 768
