...
async function exerciseConnectedBootstrapFixture(f: {
  ...
  try {
    ...
    const { exerciseHelperCaller } = await import('./helper-caller.ts');
    await exerciseHelperCaller({ admin: f.admin, engine, source: f.source, root, directory, ordinaryUrl: config.database_url });
    return;
  } catch (error) {
