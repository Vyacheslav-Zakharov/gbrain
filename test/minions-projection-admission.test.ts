...
test('real protected manifests: disabled no writes, malformed/mismatch/inaccessible refusal and deletion after enqueue',async()=>{
 ...
 try{
  ...
  for(const bad of ['{SECRET',{version:1,enabled:false},{...config,worker:{...config.worker,sourceId:'other'}}]){
   put(bad);failure='';await makeProjectionProducer(engine,['source-a'],()=>30000,deps)();expect(failure).toBe('projection_configuration_failed');expect(queued).toBe(0);
  }
  put(config);chmodSync(path,0o644);failure='';await makeProjectionProducer(engine,['source-a'],()=>30000,deps)();expect(failure).toBe('projection_configuration_failed');expect(queued).toBe(0);
...
test('enabled missing fixed manifest refuses producer and actual worker retains failure, ordinary unchanged',async()=>{
 ...
 if(s.includes("last_error='projection_configuration_failed'")||s.includes('last_error=$2'))error=p[1]??'projection_configuration_failed';
 if(s.includes('FROM public.markdown_projection_source_status'))return [{last_error:error}];return [];
 }};
 await makeProjectionProducer(engine,[source],()=>30000)();expect(error).toBe('projection_configuration_failed');expect(inserts).toBe(0);
 ...
 worker.setProjectionProducer(makeProjectionProducer(engine,[source],()=>30000));
