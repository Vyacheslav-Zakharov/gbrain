...
describe('v0.37 Lane A — defaults sweep', () => {
  ...
  test('A.0: ai/defaults.ts is the canonical source (leaf module, no SDK pulls)', async () => {
    ...
    expect(defaults.DEFAULT_EMBEDDING_MODEL).toBe('google:gemini-embedding-001');
    expect(defaults.DEFAULT_EMBEDDING_DIMENSIONS).toBe(768);
  });

  // T-11 / T-12: registry + schema defaults track gateway constants.
  test('A.1: getPGLiteSchema() default-args produce a vector(768) column', async () => {
  ...
  test('A.2: getPostgresSchema() default-args produce a vector(768) column', async () => {
  ...
  test('A.5: embedding-column registry builtin defaults to Google/768 on empty config + gateway', async () => {
    // The registry's resolution chain is cfg > gateway > DEFAULT. With
    // no cfg AND no gateway, it should fall through to the canonical
    // default (Google/768). Reset gateway first to exercise that path.
    ...
    try {
      ...
      expect(reg['embedding'].provider).toBe('google:gemini-embedding-001');
      expect(reg['embedding'].dimensions).toBe(768);
    } finally {
