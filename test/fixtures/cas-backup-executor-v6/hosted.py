...
def grammar(m,args,require):
 require(set(m)=={'schema','mode','identity','hosted','roots','allowed_roots','output','recipient','recipient_sha256','openssl_sha256','timeout_seconds','max_bytes','db','globals'},'hosted manifest fields')
 ...
 g=m['globals'];gexpected=['/usr/lib/postgresql/16/bin/pg_dumpall','--globals-only','--no-password','--host='+base+'/socket','--port=55439','--username=postgres']
 require(set(g)=={'argv','executable_sha256','runuser'} and g['argv']==gexpected,'hosted fixed globals grammar')
 require(g['runuser']==db['runuser'],'hosted globals launcher binding')
 require(isinstance(g['executable_sha256'],str) and re.fullmatch('[a-f0-9]{64}',g['executable_sha256']),'hosted globals binary pin')
