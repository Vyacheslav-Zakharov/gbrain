...
class CaptureTests(unittest.TestCase):
 ...
 def test_symlink(self):
  (self.tmp/'config'/'link').symlink_to(self.big);p=self.run_capture();self.assertEqual(p.returncode,2);self.no_complete()
 def test_fifo(self):
 ...
 def test_production_requires_authority(self):
  ...
  with patch.object(capture,'hashfile',return_value='PIN'):
   self.m['db']['executable_sha256']=self.m['recipient_sha256']=self.m['openssl_sha256']='PIN'
