...
describe('AI review canonical acceptance', () => {
  ...
  test('proposal state rolls back when the reject audit cannot commit', async () => {
    const id = await seedProposal();
    await engine.executeRaw(`ALTER TABLE ai_review_events ADD CONSTRAINT test_block_take_reject CHECK (action <> 'reject')`);
    try {
      await expect(rejectTakeProposal(engine, id, 'admin-test', 'unsupported')).rejects.toThrow();
    } finally {
      await engine.executeRaw(`ALTER TABLE ai_review_events DROP CONSTRAINT test_block_take_reject`);
    }
