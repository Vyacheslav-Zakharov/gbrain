...
export async function importFromContent(engine: BrainEngine, slug: string, content: string, opts: ContentImportOptions = {}): Promise<ImportResult> {
  ...
  await engine.transaction(async (tx) => {
    ...
    await projectContentImportCodeRefs(tx, slug, sourceId, parsed);
  });
...
/** Shared best-effort doc↔impl links. Enrolled callers run this on the ordinary
 * engine after page commit; graph writes are not part of page CAS or recovery. */
export async function projectContentImportCodeRefs(
  engine: BrainEngine, slug: string, sourceId: string | undefined,
  parsed: Pick<PreparedContentImport['parsed'], 'compiled_truth' | 'timeline'>,
) {
