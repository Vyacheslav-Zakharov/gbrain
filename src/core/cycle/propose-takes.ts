 ...
 *
 * Prompt calibration status:
 *   The historical investor-style extractor was tuned against the synthetic
 *   cat15 corpus. Those scores do not validate the later operational-control
 *   rubric. The current contract adds a privacy-safe synthetic operational
 *   meeting golden set; provider-backed semantic scoring remains a separate
 *   release gate. A version bump invalidates the page-level scan cache while
 *   preserving old proposals as audit history.
 *
 ...
 */
export const PROPOSE_TAKES_PROMPT_VERSION = 'v0.36.1.10-operational-recommendations-v1';
...
/**
 * The original extractor prompt was validated against the hand-labeled
 * synthetic corpus at test/fixtures/calibration/. Historical measurement via
 * gbrain-evals cat15 (claude-sonnet-4-6 extractor, claude-haiku-4-5 matcher):
 *
 ...
 *
 * These historical scores do not cover operational recommendations. The
 * operational meeting golden fixture pins the new hermetic contract, but a
 * provider-backed eval is still required before claiming semantic quality.
 */
...
A durable organizational or system control is a gradeable recommendation when
it prescribes what should happen in future cases and names a bounded trigger,
condition of application, or scope plus a concrete actor or control mechanism.
A recommendation is gradeable by whether future applications prove useful, effective, or appropriate;
it need not be a truth-valued prediction that literally becomes true or false.
Operational examples for Russian source prose:
- "Условия применения следует хранить как структурированные теги отдельно от текста инструкции" (recommendation)
- "При сбое интеграции система должна уведомлять дежурного для запуска процедуры восстановления" (recommendation)
- "Работы следует назначать только исполнителям, которые по системе доступа находятся на смене" (recommendation)
- "Владелец безопасности должен проверять каждое изменение привилегированного доступа до его применения" (recommendation)

A meeting sentence such as "the team approved X" reports a past event. Do not
extract that event as a Take. When X itself states a durable future-facing policy
or control, extract only the underlying normative rule, without the meeting-event wrapper.
...
- Generic statements that lack a concrete actor or control, operational mechanism,
  bounded trigger/condition or scope, evidence pointer, and decision relevance
- Do not restate a run status, HOLD state, next-step checklist, or audit narration as a Take
- Do not extract a one-off task such as a purchase, dated launch, document check, or named-owner assignment;
  those belong in task/decision systems even when they appear under a meeting heading.
  A recurring control does not become a one-off task merely because it names a responsible role
- A recommendation must name a concrete actor or control mechanism plus a bounded
  trigger, condition of application, or scope, and remain durable beyond this
  single run or report; otherwise do not extract it
- Unsupported causal blame, ROI/legal effects, availability predictions, or
  personnel judgments not entailed by PAGE PROSE
- Do not invent a measurable consequence, deadline, actor, trigger, or product name
  merely to make an otherwise vague statement pass this rubric
...
- holder       ('world' | 'people/<slug>' | 'companies/<slug>' | 'brain'; field is required).
  Holder identifies who endorses or believes the claim, not its subject. Use
  'brain' only for an unattributed collective organizational policy or control;
  use 'world' for an unattributed general prediction/judgment; use a person/company
  holder when PAGE PROSE explicitly attributes endorsement to that entity.
- weight       (number 0..1 inferred from modality/endorsement: explicitly approved durable control=0.70-0.85,
                proposed or pilot control=0.50-0.70, 'I bet'/'strong conviction'=0.70-0.85,
                'I think'/'moderate conviction'=0.50-0.70, 'maybe'/'I'd guess'=0.30-0.50)
- domain       (short tag — e.g. 'operations', 'governance', 'quality', 'security',
                'tactics', 'macro', 'hiring', 'geography', 'pricing')

Kind mapping: recommendation, judgment, or non-wager prediction => kind 'take';
explicit wager => kind 'bet'. Do not use kind to encode the claim subject.
...
export function buildExtractorPrompt(input: Parameters<ProposeTakesExtractor>[0]): string {
  const slots: Record<string, string> = {
    EXISTING_TAKES_JSON: JSON.stringify(input.existingTakes, null, 2),
    REJECTED_CLAIMS_JSON: JSON.stringify(input.rejectedClaims, null, 2),
    PAGE_BODY: input.pageBody,
  };
  return EXTRACT_TAKES_PROMPT.replace(
    /\{(EXISTING_TAKES_JSON|REJECTED_CLAIMS_JSON|PAGE_BODY)\}/g,
    (_match, slot: string) => slots[slot]!,
  );
}
