...
export function verifyScheduledMarkers(rows:any[]){
 ...
 const attempts=one('scheduled.attempts.completed').rows;assert.equal(attempts.length,2);assert(attempts.every((a:any)=>a.released_at&&a.state==='reaped'&&a.job_id===final.jobId));assert.notEqual(attempts[0].run_id,attempts[1].run_id);
 ...
 const held=one('scheduled.terminal-deleted-held');assert.equal(held.scenario,'unknown-ownership');assert.notEqual(held.database,final.database);assert.deepEqual(held.jobs,[]);assert.equal(held.attempts.length,1);assert.equal(held.attempts[0].job_id,held.jobId);assert.equal(held.attempts[0].released_at,null);assert.equal(held.status.ownership.length,1);assert.equal(held.status.ownership[0].run_id,held.attempts[0].run_id);assert.equal(held.status.recovery,'operator_blocked_no_automatic_recovery');
}
