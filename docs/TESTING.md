...
- **CI matrix** (`.github/workflows/test.yml`) runs `scripts/test-shard.sh` across 10 matrix shards partitioned by weight-aware LPT bin-packing (`scripts/sharding.ts`) and INCLUDES `*.slow.test.ts` (the two outlier slow files run as dedicated jobs alongside the matrix). Each selected LPT shard is then passed to `run-unit-shard.sh` and executed in fresh small Bun batches, preventing module/mock leakage across the full ~100-file shard while retaining CI's weighted file assignment and slow coverage. CI EXCLUDES `*.serial.test.ts` from the shards and runs them in a dedicated job via `bun run test:serial`, one bun process per file. `bun run verify` gets its own job too. CI is the ground truth for "did everything pass."
