...
- postflight creates `HOLD.json` on phase failure, any cross-source proposal/canonical fingerprint change, any review event other than the exact one-to-one `system:propose_takes` supersede event paired with an allowed source-local transition, any disallowed proposal mutation/deletion, any canonical Take/Concept full-row SHA-256 change, >10 gross new Take proposal IDs, or >5 gross new Concept proposal IDs;
