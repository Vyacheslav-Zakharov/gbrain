  ...
  return <section style={studioSectionStyle('connectors')}>
    ...
    {(catalogConnectorForm.kind === 'appsheet' || catalogConnectorForm.kind === 'postgres') && <div style={{ marginTop: 14, border: '1px solid var(--border)', borderRadius: 8, padding: 10 }}>
      ...
      <div style={{ color: 'var(--text-muted)', fontSize: 12, marginBottom: 10 }}>
        Хранятся как <code>{catalogConnectorSecretConfigId()}</code> и используются Base views. Значения повторно не показываются. Здесь проверяется доступность подключения, а чтение таблицы — в Base view через Execute/Discover.
      </div>
